...
# pydevd_pycharm.settrace('166.111.81.124', port=34979, stdoutToServer=True, stderrToServer=True)
...
if __name__ == '__main__':
    ...
    if opt.test:
        ...
        trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace, optimizer=optimizer,
                          criterion=criterion, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, use_tensorboardX=False,
                          scheduler_update_every_step=True, metrics=metrics, use_checkpoint=opt.ckpt, eval_interval=50000)
        ...
        if opt.gui:
            ...
            start = time.time()
            backward_time = trainer.train(train_loader, valid_loader, max_epoch)
            end = time.time()
            print(f'total time: {end - start}s.')
            print(f'backward total time: {backward_time}s.')
            ...
            test_loader = NeRFDataset(opt, device=device, type='test').dataloader()
            trainer.evaluate(test_loader)        # ours