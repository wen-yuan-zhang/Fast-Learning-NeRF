...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1, image_prob=None, rays_mask=None):
    ''' get rays
    Args:
    ...
    if N > 0:
        ...
        if patch_size > 1:
            ...
            # TODO: 按概率采样
            # inds = sample_pixels(image_prob, N)
            # inds = torch.LongTensor(inds).cuda()
            # cuda版本
            inds = sample_pixels_torch(image_prob, N)

            # TODO: 用rays_mask筛掉loss已经比较小的光线
            inds_mask = rays_mask[inds]
            N = inds_mask.sum()
            inds = inds[inds_mask]
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        return pred_rgb, gt_rgb, loss, self.criterion(pred_rgb, gt_rgb).mean(-1)
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            self.epoch = epoch
            # TODO: 在quadtree模式下，最后100轮用全像素。直接跑原始代码时不影响。
            if epoch == max_epochs - 10:
                train_loader._data.change_to_allpixel()

            backward_time = self.train_one_epoch(train_loader)
            total_backward_time += backward_time
            # if self.workspace is not None and self.local_rank == 0:   # TODO: 为了节省时间，统一不存checkpoint
            #     self.save_checkpoint(full=True, best=False)

            if self.epoch % self.eval_interval == 0:
                self.evaluate_one_epoch(valid_loader)
                # self.evaluate_one_epoch(train_loader)             # 用全体训练集测试
                self.save_checkpoint(full=False, best=True)

        if self.use_tensorboardX and self.local_rank == 0:
            self.writer.close()

        return total_backward_time
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(step):
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                preds, truths, loss, error = self.train_step(data)      # TODO
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            self.local_step += 1
            self.global_step += 1
            self.epoch_ray_num += data['inds'].shape[1]

            self.optimizer.zero_grad()

            with torch.cuda.amp.autocast(enabled=self.fp16):
                time0 = time.time()
                preds, truths, loss, error = self.train_step(data)
                rendering_time += time.time() - time0

            time0 = time.time()
            # 这三步在概率采样和随机采样中会导致每一轮0.2s左右的差距，猜测是概率采样的loss较大导致的。这个不知道怎么解决，只能强行在数值上把两个指标拉成一致。
            self.scaler.scale(loss).backward()
            self.scaler.step(self.optimizer)
            self.scaler.update()
            ...
            # TODO
            # loader._data.update_ray_mask(data, error)   # 测试了一下，这里几乎不占时间~
        ...
        self.log(f'Mean epoch rays num {self.epoch_ray_num / self.local_step}.')
        # self.log(f'Backward time {backward_time}')
        # self.log(f'Rendering time {rendering_time}')
        self.log(f'Epoch time {time.time() - epoch_start_time}')
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_rank == 0:
                    ...
                    # cv2.imwrite(save_path_depth, pred_depth)      # TODO: 输出深度图没啥用，不用输出
