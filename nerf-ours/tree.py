...
class QuadTreeManager:
    ...
    def __init__(self, H, W, K, images, poses, mseThres=0.1, max_depth=5):
        """
        ...
        """
        self.n_images, self.h, self.w = poses.shape[0], H, W
        self.images = images
        self.epoch_size = self.n_images * self.h * self.w
        self.processor = ImageProcessor(self.images, scale=0)

        dirs = []
        origins = []

        for i in range(self.n_images):
            rays_o, rays_d = get_rays(H, W, K, poses[i])
            dirs.append(rays_d)
            origins.append(rays_o)


        self.dirs = torch.stack(dirs, 0)
        self.origins = torch.stack(origins, 0)
        ...
        for i in tqdm(range(self.n_images)):
            self.quadTrees.append(QuadTree(self.images[i].numpy(), mseThres, max_depth))
    ...
    def visualize_split_and_sample_points(self, img_id, selected_pixel):
        ...
        cv2.imwrite('debug/tree_sample_points_{}.jpg'.format(img_id), imgc[:, :, [2, 1, 0]])

    def visualize_split(self, img_id):
        imgc = self.images[img_id].numpy().copy() * 255.0
        c = get_children(self.quadTrees[img_id].root)
        for n in c:
            # Draw a rectangle
        ...
        cv2.imwrite('debug/tree_split_{}.jpg'.format(img_id), imgc[:, :, [2, 1, 0]])

    def gen_rays_v3(self, down_scale=16, debug=False, last_epoch=False):
        """
        ...
        for img_i in tqdm(range(self.n_images)):
            ...
            for leaf_id, child in enumerate(self.childrens[img_i]):
                ...
                img_normalized_pixel.append(normalized_pixel)
                self.result_leaf_id.append(torch.Tensor([[img_i, leaf_id]]).repeat([ray_num, 1]))

            img_normalized_pixel = torch.cat(img_normalized_pixel, 0).unsqueeze(0).unsqueeze(2)  # [1, leaf_ray_num, 1, 2]

            # [2 or 3, leaf_num]
            interpolated_rgb = F.grid_sample(self.images[img_i:img_i + 1].permute(0, 3, 1, 2),
                                             img_normalized_pixel).squeeze()
        ...
        return result_origin, result_dir, result_rgb

    def gen_rays_v3_1(self, down_scale=16, debug=False, last_epoch=False):
        """
        v3的只采整数像素点的单线程版本。
        :param down_scale: 下采样光线的倍数
        :param debug: 如果设置为true，在处理时保存所有采样像素点坐标，用于可视化
        ...
        for img_i in tqdm(range(self.n_images)):
            ...
            for leaf_id, child in enumerate(self.childrens[img_i]):
                if child.area > self.quadTrees[img_i].minArea + 0.01:
                    ...
                    ray_num = int(child.area * ray_num_per_pixel)       # 最底层叶子节点的光线数由面积*平均每个像素采样点数来决定
                selected_pixel_x = torch.randint(math.ceil(child.x0), math.ceil(child.x1), (ray_num,)).cpu()
                selected_pixel_y = torch.randint(math.ceil(child.y0), math.ceil(child.y1), (ray_num,)).cpu()
                seleted_pixel = torch.stack([selected_pixel_x, selected_pixel_y], 1)
                ...
                self.result_leaf_id.append(torch.Tensor([[img_i, leaf_id]]).repeat([ray_num, 1]).cpu())

            all_selected_pixel = torch.cat(all_selected_pixel, 0)
            interpolated_rgb = self.images[img_i][all_selected_pixel[:, 0], all_selected_pixel[:, 1], :].transpose(0, 1)
            interpolated_dir = self.dirs[img_i][all_selected_pixel[:, 0], all_selected_pixel[:, 1], :].transpose(0, 1)
            interpolated_origin = self.origins[img_i][all_selected_pixel[:, 0], all_selected_pixel[:, 1], :].transpose(0, 1)
        ...
        result_rgb = torch.cat(result_rgb, 1).transpose(0, 1)
        ray_num = result_rgb.shape[0]
        shuffle_index = torch.randperm(ray_num).cpu()
        result_rgb = result_rgb[shuffle_index].contiguous()
        result_dir = torch.cat(result_dir, 1).transpose(0, 1)[shuffle_index].contiguous()
        result_origin = torch.cat(result_origin, 1).transpose(0, 1)[shuffle_index].contiguous()
        # [N, 2], N采样像素个数，2分别表示图片id、叶子节点编号
    ...
    def gen_rays_v3_multiThread(self, down_scale=16, prob=True, randSamp_proc=0.95, debug=False, last_epoch=False):
        """
        gen_rays_v3的多线程版本
        """
        ray_num_per_image = self.epoch_size / self.n_images / down_scale
        ...
        if last_epoch:
            ...
            for tree_id in range(self.n_images):
                thread_vars.append((None, {
                    ...
                    'result_leaf_id': result_leaf_id, 'prob': prob, 'randSamp_proc': randSamp_proc, 'debug': debug
                }))
        else:
            for tree_id in range(self.n_images):
                thread_vars.append((None, {
        ...
        pool = threadpool.ThreadPool(1)     # 7 !
        # TODO: v3非整数点采样，v3_1整数点采样
        requests = threadpool.makeRequests(gen_rays_v3_1_subThread, thread_vars)
        [pool.putRequest(req) for req in requests]
        pool.wait()

        result_rgb = torch.cat(result_rgb, 0)
        result_dir = torch.cat(result_dir, 0)
        result_origin = torch.cat(result_origin, 0)
        result_leaf_id = torch.cat(result_leaf_id, 0)

        shuffle_index = torch.randperm(result_rgb.shape[0])

        result_rgb = result_rgb[shuffle_index]
        result_dir = result_dir[shuffle_index]
        result_origin = result_origin[shuffle_index]
    ...
    def gen_rays_v4(self, sampler_ret, down_scale=1, debug=False):
        ...
        for img_i in tqdm(range(self.n_images)):
            ...
            if debug:
                result_selected_pixel = sampler_ret[img_i]
                self.visualize_split_and_sample_points(img_i, result_selected_pixel)
        ...
        shuffle_index = torch.randperm(ray_num)
        result_rgb = result_rgb[shuffle_index]
        result_dir = torch.cat(result_dir, 0)[shuffle_index]
        result_origin = torch.cat(result_origin, 0)[shuffle_index]
        # [N, 2], N采样像素个数，2分别表示图片id、叶子节点编号
        self.result_leaf_id = torch.cat(self.result_leaf_id, 0)[shuffle_index]
    ...
    def adjust_tree_multiThread(self, rgb_gt, rgb_pred, thres=0.001, debug=False):
        """
...
def gen_rays_v3_1_subThread(manager: QuadTreeManager, root: QuadTree, tree_id: int, children, ray_num_per_pixel: int,
                          ...
                          result_leaf_id: List, prob: bool, randSamp_proc: float, debug: bool):
    """
    ...
    for leaf_id, child in enumerate(children):
        ...
        if prob:
            # TODO: 一大半光线用概率方法采，一小半光线随机采
            ray_num1 = int(ray_num * (1-randSamp_proc))
            ...
            all_selected_pixel.append(seleted_pixel)
        # selected_pixel_x = torch.randint(math.ceil(child.x0), math.ceil(child.x1), (ray_num,))
        # selected_pixel_y = torch.randint(math.ceil(child.y0), math.ceil(child.y1), (ray_num,))
        # seleted_pixel = torch.stack([selected_pixel_x, selected_pixel_y], 1)
    ...
    if False:
        ...
        manager.visualize_split(tree_id)

    # [tree_ray_num, 2 or 3]
    interpolated_rgb = manager.images[tree_id][all_normalized_pixel[:, 0], all_normalized_pixel[:, 1], :]
    interpolated_dir = manager.dirs[tree_id][all_normalized_pixel[:, 0], all_normalized_pixel[:, 1], :]
    interpolated_origin = manager.origins[tree_id][all_normalized_pixel[:, 0], all_normalized_pixel[:, 1], :]
