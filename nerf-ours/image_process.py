...
class ImageProcessor:
    def __init__(self, images, scale=50):
        """
        ...
        """
        self.scale = scale
        self.n_images, self.h, self.w = len(images), images[0].shape[0], images[0].shape[1]
        self.images = images

        if torch.is_tensor(self.images):
            self.images_np = self.images.numpy()
        else:
            self.images_np = self.images
