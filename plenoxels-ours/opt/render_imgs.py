...
# get epoch number from ckpt name
test_epoch = args.ckpt[-6:-4]
if test_epoch == 'pt':
    test_epoch = '_last'
else:
    test_epoch = '_' + test_epoch

render_dir = path.join(path.dirname(args.ckpt),
            'train_renders'+test_epoch if args.train else 'test_renders'+test_epoch)
want_metrics = True
if args.render_path:
...
# TODO: marching cubes
# from visual import reconstruct
# reconstruct.extract_mesh(grid.links, grid.density_data, filedir=args.ckpt[:-9])  # 去掉最后的ckpt.npz
