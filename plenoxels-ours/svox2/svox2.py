...
class SparseGrid(nn.Module):
    ...
    def __init__(
        ...
        # 初始化时，把物体限制在半径为1的球内，把这些体素块作为稀疏索引
        if use_sphere_bound:
            ...
            self.capacity = n3
        # density_data存节点的density，sh_data存节点的spherical harmony参数
    ...
    def resample(
        ...
        with torch.no_grad():
            ...
            # 细分所有体素格点之后，先对density做trilinear interpolation，
            # 然后筛出所有大于density阈值的体素块，再对这些体素块的spherical harmony做trilinear interpolation
            # 这样就完成了细分、剪枝和新体素格点的插值
